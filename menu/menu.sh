...
echo -e "$COLOR1 UPTIME        $WH:  $uptime""$COLOR1 DOMAIN        $WH:  $(cat /etc/xray/domain)"
...
echo -e "$COLOR1 MASA AKTIF    $WH:  $days_remaining hari""$COLOR1 Your Ping     $WH:  $ping \e[037;1m [ $url ]"
echo -e "$COLBG1                 $WH[🛠️] STATUS SERVICE                  $COLBG1"
echo -e "$COLOR1 SSH     :$status_ws""  NGINX     :$status_nginx""  XRAY    :$status_xray""  HAPROXY : $status_haproxy"
echo -e "$COLOR1 NOOBZ :$stat_noobz"  DROPBEAR  :$status_beruangjatuh"  WS-ePro  :$status_ws_epro"  TROJANGO  :$stat_trgo"
